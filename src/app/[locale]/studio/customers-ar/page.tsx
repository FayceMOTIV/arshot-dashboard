...
import { getProducts, getCustomerARCaptures, generateSocialProofVideo, getStudioJob, IS_DEMO } from "@/lib/api";
...
const MOCK_CAPTURES: CustomerARCapture[] = IS_DEMO ? [
...
const MOCK_MODELS: ARModel[] = IS_DEMO ? [
  {
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        ...
        {/* Video Preview */}
        {currentJob?.status === "done" && (currentJob.videoUrl || models[0]?.glbUrl) && (
          <VideoPreview
            videoUrl={currentJob.videoUrl || ""}
            productId={selectedProduct === "all" ? models[0]?.id || "" : selectedProduct}
            jobId={currentJob.id}
            glbUrl={models[0]?.glbUrl ?? undefined}
