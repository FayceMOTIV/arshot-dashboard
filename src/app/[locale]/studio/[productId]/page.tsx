...
import { getProductStatus, getStudioTrends, generateStudioVideo, getStudioJob, IS_DEMO } from "@/lib/api";
...
const MOCK_MODEL: ARModel | null = IS_DEMO ? {
  id: "1",
  userId: "u1",
...
const MOCK_TREND: TrendData | null = IS_DEMO ? {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        setModel(modelData);
        setTrend(trendData);
        if (!modelData) toast.error(t("videoFailed"));
      } catch {
        setModel(MOCK_MODEL);
        setTrend(MOCK_TREND);
      } finally {
      ...
      <div className="space-y-6">
        ...
        {!currentJob && (
          <div className="flex justify-center">
            <Button
              size="lg"
              className="gap-2 bg-[#C2410C] hover:bg-[#9A3412] text-white px-8"
        ...
        {/* Video Preview */}
        {currentJob?.status === "done" && (currentJob.videoUrl || model?.glbUrl) && (
          <VideoPreview
            videoUrl={currentJob.videoUrl || ""}
            productId={productId}
            jobId={currentJob.id}
            glbUrl={model?.glbUrl ?? undefined}
            template={selectedTemplate ?? currentJob.template}
